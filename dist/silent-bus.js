    ...
    for (; p < m; ) {
      ...
      if (null !== t) {
        if ((c++, 1 === t.nodeType)) {
          ...
          "TEMPLATE" === t.tagName && (n.push(t), (o.currentNode = t.content));
          ...
          if (e.indexOf(s) >= 0) {
            ...
            "" === n[o] ? (s.insertBefore(d(), t), i.push(t)) : (t.data = n[o]),
              (p += o);
          }
        } else if (8 === t.nodeType)
          if (t.data === s) {
            const e = t.parentNode;
            (null !== t.previousSibling && c !== l) ||
              (c++, e.insertBefore(d(), t)),
              (l = c),
              this.parts.push({ type: "node", index: c }),
...
function c(t, e) {
  ...
  for (; n.nextNode(); ) {
    ...
    for (
      ...
      (o.index = null !== h ? -1 : o.index - l), (r = u(s, r)), (o = s[r]);
  }
    ...
    for (; o < s.length; )
      if (((r = s[o]), l(r))) {
        for (; a < r.index; )
          a++,
            "TEMPLATE" === d.nodeName &&
              (i.push(d), (n.currentNode = d.content)),
            null === (d = n.nextNode()) &&
              ((n.currentNode = i.pop()), (d = n.nextNode()));
        if ("node" === r.type) {
...
class C {
  ...
  setValue(t) {
    t === f ||
      (w(t) && t === this.value) ||
      ((this.value = t), g(t) || (this.committer.dirty = !0));
  }
  commit() {
    for (; g(this.value); ) {
      const t = this.value;
...
const k = (t) =>
  t &&
  (L ? { capture: t.capture, passive: t.passive, once: t.once } : t.capture);
function D(t) {
  let e = M.get(t.type);
  void 0 === e &&
...
const O = new (class {
  handleAttributeExpressions(t, e, i, s) {
    const n = e[0];
    if ("." === n) {
      return new P(t, e.slice(1), i).parts;
    }
    return "@" === n
      ? [new $(t, e.slice(1), s.eventContext)]
      : "?" === n
      ? [new N(t, e.slice(1), i)]
      : new x(t, e, i).parts;
  }
...
const q = {
  ...
  I = (t, e) => e !== t && (e == e || t == t),
  H = { attribute: !0, type: String, converter: q, reflect: !1, hasChanged: I };
class J extends HTMLElement {
  ...
  static _valueHasChanged(t, e, i = I) {
    return i(t, e);
  }
  ...
  static _propertyValueToAttribute(t, e) {
    ...
    const i = e.type,
      s = e.converter;
    return ((s && s.toAttribute) || q.toAttribute)(t, i);
  }
  initialize() {
  ...
  _requestUpdate(t, e) {
    ...
    if (void 0 !== t) {
      ...
      s._valueHasChanged(this[t], e, n.hasChanged)
  ...
  (X.render = (t, e, s) => {
    ...
    if (
      (((t, e, s) => {
        ...
        void 0 === n &&
          (i(e, e.firstChild),
          E.set(e, (n = new T(Object.assign({ templateFactory: D }, s)))),
          n.appendInto(e)),
          n.setValue(t),
          n.commit();
      })(t, l, Object.assign({ templateFactory: z(n) }, s)),
      a)
    ) {
      const t = E.get(l);
      E.delete(l);
      const s = t.value instanceof y ? t.value.template : void 0;
      F(n, l, s), i(e, e.firstChild), e.appendChild(l), E.set(e, t);
    }
...
let tt = class extends X {
  constructor() {
    ...
    const t = this.getAttribute("station"),
      e = JSON.parse(this.getAttribute("busLines")),
      i = JSON.parse(this.getAttribute("fromStation")),
      s = JSON.parse(this.getAttribute("toStation"));
    if (
      (t &&
        e &&
        ((this.busLines = e),
      ...
      const t = { fromStation: i, toStation: s, isTrain: !0 };
      this.setConfig(t);
    }
  }
  setConfig(t) {
    this.validateConfig(t),
      (this.busLines = t.busLines),
      (this.station = t.station),
      (this.isBus = !t.isTrain),
      (this.fromStation = t.fromStation),
      (this.toStation = t.toStation),
      this.fetchData();
  }
  validateConfig(t) {
    if (!(t.isTrain || (t.station && t.busLines)))
      throw new Error("You need to pass station & busLines");
    if (t.isTrain && (!t.fromStation || !t.toStation))
      throw new Error("You need to pass from and to stations");
    if (
      t.isTrain &&
      (!this.trainStations[t.fromStation] || !this.trainStations[t.toStation])
    )
      throw new Error("Didnt find this station number");
  }
  connectedCallback() {
    super.connectedCallback(),
  ...
  async fetchDataTrains() {
    var t;
    if (!this.fromStation || !this.toStation) return;
    const e = `https://silent-be.onrender.com/trains?fromStation=${this.fromStation}&toStation=${this.toStation}`,
      i = this.fetchTrainsFromCache();
    if (
      i &&
      this.isDateToday(
        null === (t = i[0]) || void 0 === t ? void 0 : t.departureTime
      )
    )
      return (
        (this.trains = i), (this.isTrainLoading = !1), void this.requestUpdate()
      );
    this.cleanOldCache(),
      (this.isTrainLoading = !0),
      fetch(e, { headers: { key: this.genKey() } }).then(async (t) => {
        var e;
        const i = await t.json();
        (this.trains =
          null === (e = i.result) || void 0 === e ? void 0 : e.travels),
          this.trains &&
            this.trains.length &&
            localStorage.setItem(
              this.getStorageKey(),
              JSON.stringify(this.trains)
            ),
          (this.isTrainLoading = !1),
          this.requestUpdate();
      });
  }
  cleanOldCache() {
    const t = Object.keys(localStorage).find((t) =>
      t.includes(`${this.fromStation}-${this.toStation}`)
    );
    t && localStorage.removeItem(t);
  }
  getStorageKey() {
    if (!this.fromStation || !this.toStation) return "";
    const t = new Date().toLocaleDateString(void 0, {
      day: "numeric",
      month: "numeric",
    });
    return `${this.fromStation}-${this.toStation}-${t}`;
  }
  fetchTrainsFromCache() {
    return JSON.parse(localStorage.getItem(this.getStorageKey()));
  }
  fetchData() {
    if ((this.genKey(), !this.isBus)) return void this.fetchDataTrains();
    this.businformation = [];
    const t = Math.floor(new Date().getTime() / 1e3),
      e = `https://app.busnearby.co.il/stopSearch?query=${this.station}&locale=he`;
    fetch(e)
      .then((t) => t.json())
      .then((e) => {
        const i = `https://api.busnearby.co.il/directions/index/stops/1:${e[0].stop_id}/stoptimes?numberOfDepartures=1&timeRange=86400&startTime=${t}&locale=he`;
        return (this.stopName = e[0].stop_name), fetch(i);
      })
      .then((t) => t.json())
      .then((t) => {
        t
          .filter((t) => {
            ...
            return (
              !(
                (null ===
                  (i =
                    null === (e = t.pattern.destStop) || void 0 === e
                      ? void 0
                      : e.code) || void 0 === i
                  ? void 0
                  : i.toString()) === this.station.toString()
              ) &&
              t.times.find((t) =>
                this.busLines
                  .map((t) => t.toString())
                  .includes(t.routeShortName)
              )
            );
          })
          .forEach((t) => {
            const e = t.times[0],
              i = new Date(1e3 * e.serviceDay),
              s = new Date(),
              n = i.getDate() !== s.getDate();
            this.businformation.push({
              lineNumber: e.routeShortName,
              realTimeArrivalDate: this.secondsToDateOriginal(
                e.realtimeArrival,
                n
              ),
              realTimeArrivalFromNow: this.getArrivalTimeDiff(
                e.realtimeArrival,
                n
              ),
              realTimeArrivalDelay: e.arrivalDelay / 60,
              isRealTime: e.realtime,
              destination: t.pattern.destStop.city,
              agency: t.pattern.route.agency,
            });
          }),
          this.requestUpdate(),
  ...
  secondsToDateOriginal(t, e = !1) {
    const i = new Date();
    return e && i.setDate(i.getDate() + 1), i.setHours(0, 0, t), i;
  }
  getArrivalTimeDiff(t, e = !1) {
    const i = new Date();
    return (
      (this.secondsToDateOriginal(t, e).getTime() - i.getTime()) / 1e3 / 60
    );
  }
  getShortTime(t) {
    if (!t) return "";
    const e = null == t ? void 0 : t.split("T")[1],
      [i, s] = null == e ? void 0 : e.split(":");
    return `${i}:${s}`;
  }
  isDateToday(t) {
    if (!t) return "";
  ...
  getRealTimeArrivalFromNowText(t) {
    return t >= 120
      ? `${Math.round(t / 60)} hours`
      : t >= 1 && t <= 120
      ? `${Math.round(t)} min`
      : `${60 * t} sec`;
  }
  getDelayMessage(t) {
    const e = t >= 0 ? "delay" : "earlier";
    return t >= 60 || 0 === t
      ? ""
      : t >= 1
      ? `${Math.round(t)} min ${e}`
      : `${Math.round(Math.abs(60 * t))} sec ${e}`;
  }
  refresh() {
    this.fetchData(), this.isBus && this.handleRefreshClickForBus();
  }
  genKey() {
    ...
    for (let s = 0; s < t.length; s++) {
      const n = t.charCodeAt(s) ^ e.charCodeAt(s % e.length);
      i += String.fromCharCode(n);
    }
    return i;
  }
  handleRefreshClickForBus() {
    const t = this.shadowRoot.querySelector(".refresh");
  ...
  getColorByAgency(t) {
    switch (t) {
      case "3":
        return "rgb(1, 131, 105)";
      case "15":
        return "rgb(247, 143, 30)";
      default:
        return "blue";
    }
  }
  getLastUpdated() {
    var t;
    return null === (t = this.lastUpdated) || void 0 === t
      ? void 0
  ...
  rednerBus() {
    return U`
      ...
      <div>
        ${this.businformation.map(
          (t) => U`
            <div class="item">
              <div class="main-row">
                <div
                  class="lineNumber"
                  style="background-color: ${this.getColorByAgency(
                    t.agency.id
                  )}"
                >
                  ${t.lineNumber}
                ...
                <div class="time">
                  ...
                  <div>
                    ${this.getRealTimeArrivalFromNowText(
                      t.realTimeArrivalFromNow
                    )}
                  </div>
                </div>
                <div
                  class="delay"
                  style="color: ${
                    t.realTimeArrivalDelay >= 0 ? "red" : "green"
                  }"
                >
              ...
              <!--                </div>-->
              <!--                <div>-->
              <!--                  Real time arrival delay: ${
                t.realTimeArrivalDelay
              }-->
              <!--                  minutes-->
              <!--                </div>-->
              <!--                <div>-->
    ...
    return U`
      ...
      <div class="station-name">
        <span
          ...
          <span style="margin: 0 10px">לכיוון</span> ${
            this.trainStations[this.toStation]
          }</span
        >
      </div>
      <span class="${
        this.isTrainLoading ? "" : "trainHidden"
      ...
      <div class="${
        ...
        <span style="direction: rtl; font-size: 16px"
          >(עוד ${this.getTrainDiffTime(
            null === (t = this.getFirstTrain()) || void 0 === t
              ? void 0
              : t.departureTime
          )}
          דק)</span 
        ...
        <span style="margin: 5px 10px; font-size: 32px; color: blue">
          ${this.getShortTime(
            null === (e = this.getFirstTrain()) || void 0 === e
              ? void 0
              : e.departureTime
          )}</span
        >
  ...
  getFirstTrain() {
    var t, e;
    const i =
      null === (t = this.trains) || void 0 === t
        ? void 0
        : t.find((t) => {
            const e = t.departureTime,
              i = new Date();
            return new Date(e) > i;
          });
    return null === (e = null == i ? void 0 : i.trains) || void 0 === e
      ? void 0
      : e[0];
  }
  getNextTrains() {
    ...
    return (
      (e = null == e ? void 0 : e.splice(1, 2)),
      null == e
        ? void 0
        : e.map((t) => this.getShortTime(t.departureTime)).join(", ")
    );
  }
  getSignalSvg() {
    return U`<svg
...
(tt.styles = ((t, ...e) => {
  ...
  `),
  t([W()], tt.prototype, "config", void 0),
  t([W()], tt.prototype, "hass", void 0),
  t([W()], tt.prototype, "station", void 0),
  (tt = t(
    [
      ((et = "silent-bus"),
      (t) =>
        "function" == typeof t
          ? ((t, e) => (window.customElements.define(t, e), e))(et, t)
          : ((t, e) => {
  ...
  customElements.define("silent-bus", tt),
